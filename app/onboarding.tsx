...
export default function OnboardingScreen() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (!user?.user_id) { setError('Sleeper account not found. Check your username.'); triggerShake(); setLoading(false); return; }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.inner, { paddingTop: insets.top + 40, paddingBottom: insets.bottom + 24 }]}>
          ...
          <Animated.View style={[styles.card, { transform: [{ translateX: shake }] }]}>
            ...
            <TouchableOpacity style={[styles.button, loading && { opacity: 0.7 }]} onPress={handleConnect} disabled={loading}>
              {loading
...
const styles = StyleSheet.create({
  ...
  logoAIO:     { fontFamily: F.bold, fontSize: 72, color: C.blueDeep, letterSpacing: 4, lineHeight: 72 },
  logoMni:     { fontFamily: F.bold, fontSize: 36, color: C.blueDeep, letterSpacing: 6, marginBottom: 8, opacity: 0.55 },
  logoDivider: { width: 36, height: 2, backgroundColor: C.gold, marginVertical: 14, opacity: 0.6 },
  ...
  card: {
    ...
    borderWidth: 1.5, borderColor: BORDER, marginBottom: 24,
  ...
  inputWrap:      { flexDirection:'row', alignItems:'center', backgroundColor:'rgba(88,131,191,0.06)', borderRadius:12, borderWidth:1.5, borderColor:BORDER, marginBottom:12, paddingHorizontal:14 },
  inputWrapError: { borderColor:'rgba(168,48,64,0.5)' },
  inputPrefix:    { fontFamily:F.bold, fontSize:20, color:C.dim2, marginRight:6 },
  input:          { flex:1, color:C.ink, fontFamily:F.mono, fontSize:SZ.base, paddingVertical:14 },
  errorText:      { fontFamily:F.mono, color:'#a83040', fontSize:SZ.xs, letterSpacing:1, marginBottom:12 },
