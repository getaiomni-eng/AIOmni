...
function formatForPrompt(players: any[]): string {
  ...
  const lines = players.map(p => {
    ...
    return `${p.name} (${p.position}, ${p.team}, Age ${p.age}): ${p.games} games, ${p.total_points} PPR pts — ${statLine}${snap}${dynVal}`;
  });

  return `\nPLAYER INTELLIGENCE (${players.length} players, 2024 season):\n${lines.join('\n')}`;
}
